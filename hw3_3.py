...
class DAG:
    ...
    def check_if_dag(self):
        # checking parent nodes for cycles
        for i in self.graph:
            visited = set()
            self.check_if_dag_helper(i, visited)
        return self.is_dag
        ...
        try:
            for child in self.graph[node]:
                if child not in visited:
                    ...
                    for reverse_child in self.graph[child]:
                        if reverse_child == node:
                            self.is_dag = False
                            print("Graph is not a DAG. becuase of ", child, " is a child of ", node)
                            return
        except KeyError:
            pass       
